...
public class MenuScript : MonoBehaviour {
	...
	void Start ()
	{
		...
		menuPanel = transform.Find("PanelSetting");
		...
		for(int i = 0; i < menuPanel.childCount; i++)
		{
            ...
            else if (menuPanel.GetChild(i).name == "ShotKey")
                menuPanel.GetChild(i).GetComponentInChildren<Text>().text = GameManager.GM.shot.ToString();
        }
	}
	...
	public IEnumerator AssignKey(string keyName)
	{
		...
		case "jump":
			...
			break;
        case "shot":
            GameManager.GM.shot = newKey; //set jump to new keycode
            buttonText.text = GameManager.GM.shot.ToString(); //set button text to new key
            PlayerPrefs.SetString("shotKey", GameManager.GM.shot.ToString()); //save new key to playerprefs
            break;
        }
